...
public class GameController {
    ...
    public void onMoveTokens(Token token, Position newPos, int throwResult) { game.applyMoveTo(token, newPos, throwResult); }
    public void onStackTokens(Token tokenA, Token tokenB) { game.handleStacking(tokenA, tokenB); }
    public void onCaptureTokensYes(Token tokenA, Token tokenB) { game.handleCapturing(tokenA, tokenB); }

    /* 게임 흐름 로직
     * onExitYes(): 게임 종료 버튼 누르면 실행 
     * onRestartYes(): 재시작 버튼 누르면 실행
     */
    public void onExitYes() { game.endGame(); } // TODO: 게임 종료 거절 버튼을 누르면 UI상에서 뒤로가기만 수행하면 될 것 같아요
    public void onRestartYes() { game.restartGame(); }
