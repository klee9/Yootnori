...
public class Board {
    ...
    public void initBoard(String shape) {
        // add positions
        int numCorners = 0;
        switch (shape) {
            case "사각형" -> numCorners = 4;
            case "오각형" -> numCorners = 5;
            case "육각형" -> numCorners = 6;
        }
        for (int i = 0; i < 7*numCorners + 1; i++) {
            positions.add(new Position(i, numCorners));
        }
